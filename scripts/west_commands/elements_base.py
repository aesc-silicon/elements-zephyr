...
VERILATOR_ROOT_DIR = ZEPHYR_ROOT_DIR / Path("../verilator/")
...
class Elements:
    ...
    def get_env(self):
        ...
        env['PATH'] = str(VERILATOR_ROOT_DIR / "bin") + os.pathsep + env['PATH']
        env['VERILATOR_ROOT'] = str(VERILATOR_ROOT_DIR)
        return env
    ...
    def synthesize(self):
        ...
        flow = {
            ...
            "dependencies": {
                ...
                "synth_log": f"{build}/f4pga/synth.log",
                "pack_log": f"{build}/f4pga/pack.log"
            },
            self.meta['part']: {
                "default_target": "bitstream",
                "dependencies": {
                    "build_dir":  str(build / "f4pga"),
                    "xdc": [
        ...
        command = f"{ZIBAL_ROOT_DIR}/eda/{self.meta['vendor']}/f4pga/syn.sh"
        env = self.get_env()
        cwd = build / "f4pga"
        ...
        self.check_call(f"ln -sf {build}/f4pga/{top}.bit {build}/{top}.bit", env, cwd)
    ...
    def simulate(self):
        ...
        command = f"sbt \"runMain elements.soc.{soc.lower()}.{board}Simulate\""
        env = self.get_env()
        cwd = ZIBAL_ROOT_DIR
        self.check_call(command, env, cwd)
