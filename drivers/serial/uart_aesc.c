...
struct uart_aesc_config {
	DEVICE_MMIO_ROM;
	/* clock_dev set => rate comes from the clock controller; otherwise the
	 * static sys_clk_freq (clock-frequency DT property) is used.
	 */
...
/* Prefer the clock controller (clocks phandle); fall back to clock-frequency. */
