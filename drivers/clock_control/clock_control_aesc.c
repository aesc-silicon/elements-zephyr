...
{
	struct clock_control_aesc_data *data = DEV_DATA(dev);
	uint32_t index = (uint32_t)(uintptr_t)sys;
	uintptr_t reg;

	if (index >= clock_control_aesc_domain_count(dev)) {
		return -EINVAL;
	}

	reg = data->reg_base + CLOCK_AESC_DOMAIN_CONTROL(index);
				  ...
				  clock_control_subsys_t sys)
{
	...
	reg = data->reg_base + CLOCK_AESC_DOMAIN_CONTROL(index);
	sys_write32(0, reg);
	/* Domains the SoC declares as non-gateable hardwire the enable bit. */
	if ((sys_read32(reg) & CLOCK_CTRL_ENABLE) != 0) {
		return -ENOTSUP;
	}
				       ...
				       uint32_t *rate)
{
	...
	struct clock_control_aesc_data *data = DEV_DATA(dev);
	uint32_t index = (uint32_t)(uintptr_t)sys;
	uint32_t reference, ratio, mult, div;
	int ret;
	...
	ratio = sys_read32(data->reg_base + CLOCK_AESC_DOMAIN_RATIO(index));
	...
	*rate = (uint32_t)(((uint64_t)reference * mult) / div);
				...
				clock_control_subsys_t sys)
{
	...
	control = sys_read32(data->reg_base + CLOCK_AESC_DOMAIN_CONTROL(index));
